package com.sapient.survival.controller;

import java.io.IOException;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.sapient.survival.dao.BlockDao;
import com.sapient.survival.exceptions.InvalidBlockUpdate;

public class BlockStatusController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public BlockStatusController() {
		super();

	}

	protected void doGet(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		doPost(request, response);
	}

	protected void doPost(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		String blocks[] = request.getParameterValues("blockId");
		BlockDao obj = new BlockDao();
		int res = 0;
		try {
			res = obj.sendBlockForExecution(blocks);
		} catch (InvalidBlockUpdate e) {
			System.out.println(e.getMessage());
		}
		if (res == 1) {
			RequestDispatcher rd = request.getRequestDispatcher("/viewBlocks");
			rd.forward(request, response);
		}
	}

}
